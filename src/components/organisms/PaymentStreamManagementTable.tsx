...
export default function PaymentStreamManagementTable() {
    ...
    return (
        <div className="bg-[#1E212F]  pr-4 rounded-xl  max-w-[370px] md:w-full md:max-w-[1136px] overflow-x-auto min-w-sm " >
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th className="min-w-[110px] w-[150px] h-[72px] text-center bg-[#1E212F] p-[10px] text-xs  md:text-base font-bold text-[#E1E1E1]">Action</th>
                    </tr>
                </thead>
                        ...
                        return (
                            <tr key={index} className="border-t-[2px] border-b-[1px] border-[#CDCDCD0D]"   >
                                ...
                                <td className="text-center h-[72px]  bg-[#1E212F] p-[10px]  text-xs md:text-base font-normal" >
                                    <span
                                        className={`py-[6px] px-[8px] bg-[#F4FBF6] block text-center text-red-600 rounded-lg border-[1px] w-fit self-center min-w-[64px] mx-auto
                 ${stream.status === "Active" || stream.status === "Completed" ? "border-[#2B9A66] text-[#2B9A66] " : "text-[#DC3E42] border-[#FFDBDC] "} `} >
                                        {stream.status}
                                    </span> </td>

                                <td className="flex items-center h-[72px] justify-center gap-[27px]  bg-[#1E212F] p-[10px]  " >
                                    <div className={`  overflow-hidden flex items-center justify-between gap-[27px] transition-all duration-150 ease-in-out ${openMenuIndex === index ? "w-full" : "w-0"} `}  >
                                        <button onClick={() => setOpenMenuIndex(null)} ><Trash /></button>
                                        <button onClick={() => setOpenMenuIndex(null)} ><Trash /></button>
                                    </div>
